...
def run_silver_merge(
    ...
    *,
    ...
    When ``since_ingested_at`` is set, only bronze rows with ``_ingested_at`` after
    that timestamp are read (incremental). When omitted, the full bronze table is
    scanned (useful for backfills).
    """
